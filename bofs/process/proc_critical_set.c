/*
 * proc_critical_set.c — Set a process as critical (system) process.
 *
 * When a critical process is terminated, Windows triggers a BSOD.
 * Uses NtSetInformationProcess with ProcessBreakOnTermination (0x1D).
 *
 * Args (optional): "<pid>"
 *   If PID is omitted, defaults to the current process.
 *
 * Requires SeDebugPrivilege for targeting other processes.
 *
 * Build:
 *   x86_64-w64-mingw32-gcc -c proc_critical_set.c -o proc_critical_set.o
 */
...
static void enable_debug_priv(void) {
    ...
    /* TOKEN_PRIVILEGES: count(4) + LUID(8) + attributes(4) = 16 bytes */
    char tp[16];
    bof_memset(tp, 0, 16);
    *(DWORD*)tp = 1; /* PrivilegeCount */
...
void go(char* args, int alen)
{
    ...
    /* Set ProcessBreakOnTermination = TRUE */
    ULONG val = 1;
    ...
    if (status == 0) {
        op = sa(out, op, "[Critical] Process set as CRITICAL (system process)\n");
        ...
        op = sa(out, op, "  BSOD will occur if this process is terminated.\n");
