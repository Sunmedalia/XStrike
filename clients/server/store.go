...
// ---- agent sysinfo (recon BOF output) ----

// PutSysinfo stores the parsed sysinfo fields (KEY=VALUE) for an implant as a
// JSON blob, replacing any prior row. Called when the auto-run sysinfo BOF's
// output arrives on connect.
func (s *Store) PutSysinfo(implantID uint64, fields map[string]string) error {
	b, err := json.Marshal(fields)
	if err != nil {
		return err
	}
	_, err = s.db.Exec(`INSERT INTO agent_sysinfo(implant_id, info, ts) VALUES(?,?,?)
		ON CONFLICT(implant_id) DO UPDATE SET info=excluded.info, ts=excluded.ts`,
		implantID, string(b), time.Now().Unix())
	return err
}

// GetSysinfo returns the parsed sysinfo fields for one implant (empty map if none).
func (s *Store) GetSysinfo(implantID uint64) (map[string]string, error) {
	var info string
	err := s.db.QueryRow(`SELECT info FROM agent_sysinfo WHERE implant_id=?`, implantID).Scan(&info)
	...
	_ = json.Unmarshal([]byte(info), &out)
	return out, nil
}

// GetSysinfoBatch returns sysinfo for many implants in one query (used to enrich
// the /api/implants + /api/agents lists without N+1 lookups).
func (s *Store) GetSysinfoBatch(ids []uint64) (map[uint64]map[string]string, error) {
	...
	q := `SELECT implant_id, info FROM agent_sysinfo WHERE implant_id IN (` + strings.Join(placeholders, ",") + `)`
	rows, err := s.db.Query(q, args...)
	if err != nil {
		return nil, err
	}
