    ...
    if (isRealMode()) {
      ...
      unsubWails = wailsEventsOn('core:event', (ev: WailsCoreEvent) => {
        ...
        switch (ev.type) {
          ...
          case 'output':
          case 'sysinfo':
            // A session changed (or produced output / recon) — refetch the
            // list + logs. /api/implants is enriched with sysinfo fields, so
            // a sysinfo event refreshes the agent table with host recon.
