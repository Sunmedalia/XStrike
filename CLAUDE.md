...
```sh
cargo build --release             # whole workspace (server + implant + beacon + beacon-cycle + loader + protocol)
cargo test --release              # protocol + coff + beacon + beacon-cycle + server + in-process BOF exec
...
- **Opaque trailer magic** — the appended-config marker is the byte sequence
  ...
  `crates/beacon/src/lib.rs::TRAILER_MAGIC`,
  `crates/beacon-cycle/src/lib.rs::TRAILER_MAGIC`,
  `tools/stubbuilder/main.go`, and `clients/server/stub_patcher.go::TrailerMagic`.
  Change it in all five together. Trailer layout is `magic <host>\0<port>\0` for
  the implant; the beacon adds an optional third field `<interval_secs>\0`
  (backwards compatible — the implant stops after port and ignores trailing
  bytes); the beacon-cycle adds a fourth `<dwell_secs>\0` after the interval.
  `clients/server/stub_patcher.go::PatchImplant` writes all forms;
  `crates/beacon/src/lib.rs::parse_trailer_tail` reads the optional interval;
  `crates/beacon-cycle/src/lib.rs::parse_trailer_tail` reads interval + dwell.
...
Six crates in a single workspace. Dependency direction: `protocol` ← `server`; `protocol` + `loader` ← `implant` / `beacon` / `beacon-cycle`.
...
- **`implant`** — Windows reverse-connect binary (native MSVC build). A single lib (`src/lib.rs` — `run()`, reader loop, handle, trailer reader, stealth module) backs **two** bin targets: `ruststrike-implant` (console subsystem, dev) and `ruststrike-implant-silent` (GUI subsystem via `#![windows_subsystem = "windows"]` in `src/bin/silent.rs` — no console window, runs hidden in the background; use for deployed agents). Reader thread dispatches `ServerMessage` → `handle()`; `Bof` decodes and calls `run_bof`, returning `Output`/`Error`. Callback host/port resolution: (1) appended-config trailer on the exe (opaque magic + `host\0port\0` near the end — read by `read_trailer_config`; the magic is an opaque byte sequence, not a readable word, so it isn't a string-scan telltale), else (2) CLI args `<host> [port]`, defaults `127.0.0.1 4444`. The trailer lets an operator deploy a patched exe that reverse-connects with no args (`tools/stubbuilder` or `POST /api/stub/build` appends it; re-patching strips the old trailer first). The command-exec BOFs (`cmd_exec`/`powershell_exec`/`winapi_exec`) already spawn children with `CREATE_NO_WINDOW`, so the only visible window was the implant's own console — the silent variant removes it; the GUI's Generate Agent modal has a "Silent" checkbox (default on) whose `silent` body flag makes `/api/stub/build` swap the console base for its `ruststrike-implant-silent.exe` sibling.

- **`beacon`** — a beacon-style sibling of `implant` (same `protocol` + `loader`, same OPSEC cover identity). The difference is reconnection: `run()` is an infinite loop — check in (proactive `hello`), pump commands until the stream ends or the dial fails, sleep a configurable `interval` (±`jitter`), repeat. The server can close/restart/be unreachable and the beacon keeps retrying forever. Config precedence per field: CLI flag → positional → `BEACON_*` env → appended trailer → defaults (`127.0.0.1:4444`, 5s, 0% jitter). The trailer reuses the implant's opaque magic and adds an optional third field `<interval_secs>\0` (backwards compatible — the implant ignores it), so a stub built with `beacon:true` + Sleep Time checks in at the operator's cadence with no args/env. Two bin targets: `ruststrike-beacon` (console) / `ruststrike-beacon-silent` (GUI). Relay/pivot is declined (`relay_error`) — a beacon's link is intermittent (it sleeps between check-ins), so a long-lived pivot listener on it is not meaningful; use the stock implant for pivoting. See `crates/beacon/README.md`.
- **`beacon-cycle`** — a **short-cycle** sibling of `beacon` (same `protocol` + `loader`, same OPSEC cover identity). Instead of holding a persistent channel, each `cycle()` is short-lived: connect + proactive `hello` (marker `"cycle"`), drain commands for a `dwell` window (enforced via `TcpStream::set_read_timeout(dwell)`), then **actively close**, sleep `sleep` (±`jitter`), repeat forever. Traffic is periodic short pulses, not a persistent channel — the classic sleep-beacon link model. Config precedence per field: CLI flag → positional → `CYCLE_*` env → appended trailer → defaults (`127.0.0.1:4444`, sleep 5s, dwell 2s, 0% jitter). The trailer reuses the opaque magic and adds two fields `<sleep_secs>\0<dwell_secs>\0` (dwell optional → falls back to 2s). Two bin targets: `ruststrike-beacon-cycle` (console) / `ruststrike-beacon-cycle-silent` (GUI). Relay/pivot is declined (`relay_error`). Selected from the Generate Agent GUI via the `RustStrike Beacon (Short-Cycle)` agent template (see `agents/templates/`). See `crates/beacon-cycle/`.
...
- Discriminator is `type`, **snake_case** (`serde(tag="type", rename_all="snake_case")`). For the single-word variants this is identical to lowercase (`hello`/`bof`/`output`/`error`); the relay variants use the snake_case form (`relay_listen`/`relay_stop`/`relay_started`/`relay_stopped`/`relay_error`). Variants: `hello`, `bof`, `relay_listen`, `relay_stop` (server→implant); `hello`, `output`, `error`, `relay_started`, `relay_stopped`, `relay_error` (implant→server).
...
- **Relay (pivot):** server→implant `relay_listen` `{"type":"relay_listen","relay_id":"rl-xx","bind_ip":"0.0.0.0","port":0}` (port 0 = OS-assigned) / `relay_stop` `{"type":"relay_stop","relay_id":"rl-xx"}`; implant→server `relay_started` `{"type":"relay_started","relay_id":"rl-xx","bind_ip":"0.0.0.0","port":51234}` (actual bound port) / `relay_stopped` / `relay_error` `{"type":"relay_error","relay_id":"rl-xx","data":"bind: ..."}`. See "Pivot / relay" below.
...
Files: `session.go` (multi-implant session manager, one reader goroutine each; also captures BOF output as artifacts + touches the agent roster on connect), `bus.go` (event pub/sub), `ws.go` (dependency-free RFC6455 server, pushes `Event`s), `api.go` (REST routing), `boflib.go`+`boflib_api.go` (BOF library index + upload/run), `task.go` (BOF task-result store; records the BOF name for artifact routing), `protocol.go` (wire types mirroring `crates/protocol`), `store.go` (SQLite: logs/listeners/agents/artifacts, WAL, batched log writer), `listeners.go`+`listeners_api.go` (runtime TCP listener start/stop), `logs_api.go` (`/api/logs` from SQLite), `agents_api.go`+`artifacts_api.go` (`/api/agents`, `/api/agents/{id}/artifacts`), `agent_templates.go` (`/api/agent/templates` — reads `agents/templates/*.toml` so the Generate Agent dropdown is data-driven; `lookupTemplateVariant` resolves a template id to a stub variant), `stub_patcher.go`+`/api/stub/build`+`/api/stub/save` (implant/beacon/beacon-cycle exe trailer patcher).

REST: `GET /api/implants`, `POST /api/implants/{id}/hello`, `POST /api/implants/{id}/bof` (body `{bof, args}` — `bof` is a library name or raw base64 COFF, `args` base64; returns `{task_id}`), `DELETE /api/implants/{id}`, `GET/POST /api/bofs`, `POST /api/bofs/{name}/run?implant=<id>` (body `{args}`; returns `{task_id}`), `GET /api/tasks/{id}` (poll a BOF task → `{id, implant_id, status, output}`; `status` is `running` until the implant's output/error arrives, then `completed`/`failed`). **Listener mgmt:** `GET/POST /api/listeners`, `PUT /api/listeners/{id}`, `POST /api/listeners/{id}/start|stop`, `DELETE /api/listeners/{id}` (runtime start/stop of TCP implant listeners; the operator HTTP/WS listener is NOT managed — stopping it locks the operator out). **Persistence:** `GET /api/logs?limit=&implant=` (persisted event history from SQLite), `GET /api/agents` (persistent agent roster + live `online` flag), `GET /api/agents/{id}/artifacts?kind=&limit=`, `GET /api/agents/{id}/artifacts/{aid}` (per-agent captured BOF output: `file_list`/`proc_list`/`net_list` → text meta; `screenshot`/`download` → base64 blob). **Stub builder:** `POST /api/stub/build` body `{host, port, silent?, beacon?, cycle?, interval?, dwell?, template?}` → `{exe_b64}` (prebuilt base exe with a `host\0port\0[interval\0[dwell\0]]` trailer appended, base64; `silent=true` swaps the console base for the GUI-subsystem `-silent.exe` sibling so the agent runs with no console window; `beacon=true` selects the `ruststrike-beacon[-silent].exe` base — an auto-reconnecting agent that retries forever on a callback cadence; `cycle=true` selects the `ruststrike-beacon-cycle[-silent].exe` base — a short-cycle agent that makes short-lived connections on a cadence (cycle wins over beacon when both are set); `interval` is decimal seconds (beacon reconnection cadence / cycle sleep between check-ins), `dwell` is decimal seconds (cycle connection-hold window, defaults to 2), both baked into the trailer; `template=<id>` overrides the variant via the agent-template lookup in `agent_templates.go`). Base exe discovery: `RUSTSTRIKE_BEACON_CYCLE_EXE` env for cycle, `RUSTSTRIKE_BEACON_EXE` env for beacons, `RUSTSTRIKE_IMPLANT_EXE` env (or config `Paths.ImplantExe`/`baseImplantExe`) for the implant, else `<exe-dir>/<variant>.exe`, else repo-relative `../../target/release/<variant>.exe`. **Agent templates:** `GET /api/agent/templates` → `[{id, name, description, base, variant, supports[], default_sleep, default_dwell}, ...]` loaded from `agents/templates/*.toml` at startup (`RUSTSTRIKE_TEMPLATES` env overrides the dir; missing dir is non-fatal — the stub builder falls back to the `beacon`/`cycle` booleans). The Wails GUI's `BuildStubToProject` Go binding calls `/api/stub/build`, then pops the OS **Save As** dialog (`runtime.SaveFileDialog`) so the operator chooses where to save the agent exe — no fixed project path, no browser blob download. Returns the chosen path (empty = cancelled). The GUI's Generate Agent modal reads `/api/agent/templates` for the Agent Template dropdown (implant / beacon / beacon-cycle); Sleep Time and Dwell Time fields appear dynamically based on the template's `supports` (sleep / dwell); the beacon checkbox is meaningful only for the stock implant template. WS: `GET /ws` streams `{type, implant_id, data}` events (`implant_connected`/`implant_disconnected`/`hello`/`output`/`error`/`listener_changed`).

**Persistence (`store.go`).** `modernc.org/sqlite` (pure-Go, the core's first external dep). Tables: `logs`, `listeners`, `agents`, `artifacts` (schema created on first open; WAL mode). Every bus event is enqueued to a buffered channel and batch-inserted by a single writer goroutine (non-blocking — drops on overflow like the bus, never stalls the implant reader). Listener config is persisted so the TCP set survives a core restart (`Restore` re-opens active ones). Agent roster is upserted on connect (`TouchAgent`). Artifacts are captured in `session.go::readLoop`: when a BOF task's output arrives, `captureArtifact(bofName, implantID, output)` maps the BOF name to a kind — `file_list`/`proc_list`/`netstat` → text `meta`, `screenshot`/`file_download` → decoded base64 `blob` (split on the `=== SCREENSHOT:`/`=== FILE:` headers). The GUI hydrates Files/Processes/Net/Screenshots tabs from these on mount (after restart).
...
Verified end-to-end with the Rust implant: session list, hello, BOF-by-b64, BOF-by-name (nbtscan/ps/ls/cmd_exec/download/upload, plus the component-driven proc_list/proc_kill/file_list/file_download/screenshot/powershell_exec/shellcode_exec/shellcode_exec_nt/netstat), task-poll output, and live WebSocket event push all work.

**Pivot / relay (`relay.go` + `crates/implant/src/lib.rs`).** A connected implant can act as a CS-style TCP pivot: the operator starts a relay listener on it (`POST /api/implants/{id}/relay {bind_ip, port}` → mints `relay_id`, sends `relay_listen`; `GET /api/implants/{id}/relays` lists; `DELETE /api/implants/{id}/relays/{rid}` sends `relay_stop`). The implant binds `bind_ip:port` (port 0 = OS-assigned), spawns a detached accept loop, and replies `relay_started` with the actual port. Each accepted child is **spliced** onto a fresh connection the implant dials to the core (the core address is shared via a `CORE_ADDR` `OnceLock` set at `run()` start) — the child's bytes are transparent newline-JSON, so the core's `SessionManager.Accept` registers it as a normal new implant. **Long-running task can't be a BOF** (BOFs are one-shot, `handle()` is synchronous) — relay is a native `ServerMessage` handler: `handle()` binds + spawns + returns `relay_started` immediately; the accept/splice threads run for the implant's lifetime. To stop, `relay_stop` sets a `done` flag and dials the listener's own address to unblock the parked `accept()` (Rust's `TcpListener::accept(&self)` can't be force-closed while borrowed). Relay state is **transient** — a `RelayRegistry` (global, like `tasks`) tracks it in memory keyed by `implantID`→`relayID`; on implant disconnect `readLoop`'s `remove` calls `Cleanup` so no stale "running" entries survive. NOT persisted to SQLite (the listener lives in the implant process; a restart means no listener). `readLoop` intercepts `relay_started/stopped/error` before the task-correlation path, updates the registry, and publishes a normalized `relay_changed` event (the GUI's Pivot panel polls `/nodes/{id}/relays` to pick up the async port). OPSEC: a TCP listener on the parent is a detection surface — a named-pipe variant is the planned stealth upgrade.
...
- `app.go` — Wails-bound methods the frontend calls via the shim: `ListImplants`, `Hello`, `RunBofByName`/`RunBofByB64` (each returns a `task_id` string), `GetTaskResult(taskID)` (polls a BOF task → `TaskResult{status, output}`), `DropImplant`, `ListBofs`, `UploadBof`, `StartRelay`/`ListRelays`/`StopRelay` (pivot listeners on an implant). Each wraps a core REST call.
