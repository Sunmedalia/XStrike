...
- **Opaque trailer magic** — the appended-config marker is the byte sequence
  ...
  Change it in all four together. Trailer layout is `magic <host>\0<port>\0` for
  ...
  bytes). `clients/server/stub_patcher.go::PatchImplant` writes both forms;
...
Five crates in a single workspace. Dependency direction: `protocol` ← `server`; `protocol` + `loader` ← `implant` / `beacon`.
...
REST: `GET /api/implants`, `POST /api/implants/{id}/hello`, `POST /api/implants/{id}/bof` (body `{bof, args}` — `bof` is a library name or raw base64 COFF, `args` base64; returns `{task_id}`), `DELETE /api/implants/{id}`, `GET/POST /api/bofs`, `POST /api/bofs/{name}/run?implant=<id>` (body `{args}`; returns `{task_id}`), `GET /api/tasks/{id}` (poll a BOF task → `{id, implant_id, status, output}`; `status` is `running` until the implant's output/error arrives, then `completed`/`failed`). **Listener mgmt:** `GET/POST /api/listeners`, `PUT /api/listeners/{id}`, `POST /api/listeners/{id}/start|stop`, `DELETE /api/listeners/{id}` (runtime start/stop of TCP implant listeners; the operator HTTP/WS listener is NOT managed — stopping it locks the operator out). **Persistence:** `GET /api/logs?limit=&implant=` (persisted event history from SQLite), `GET /api/agents` (persistent agent roster + live `online` flag), `GET /api/agents/{id}/artifacts?kind=&limit=`, `GET /api/agents/{id}/artifacts/{aid}` (per-agent captured BOF output: `file_list`/`proc_list`/`net_list` → text meta; `screenshot`/`download` → base64 blob). **Stub builder:** `POST /api/stub/build` body `{host, port, silent?, beacon?, interval?}` → `{exe_b64}` (prebuilt base exe with a `host\0port\0[interval\0]` trailer appended, base64; `silent=true` swaps the console base for the GUI-subsystem `-silent.exe` sibling so the agent runs with no console window; `beacon=true` selects the `ruststrike-beacon[-silent].exe` base — an auto-reconnecting agent that retries forever on a callback cadence — instead of the stock implant; `interval` is decimal seconds, beacon-only, baked into the trailer as the callback cadence). Base exe discovery: `RUSTSTRIKE_BEACON_EXE` env for beacons, `RUSTSTRIKE_IMPLANT_EXE` env (or config `Paths.ImplantExe`/`baseImplantExe`) for the implant, else `<exe-dir>/<variant>.exe`, else repo-relative `../../target/release/<variant>.exe`. The Wails GUI's `BuildStubToProject` Go binding calls `/api/stub/build`, then pops the OS **Save As** dialog (`runtime.SaveFileDialog`) so the operator chooses where to save the agent exe — no fixed project path, no browser blob download. Returns the chosen path (empty = cancelled). The GUI's Generate Agent modal has "Silent" (default on) and "Beacon (auto-reconnect)" checkboxes plus a Sleep Time field; Sleep Time is threaded as the beacon `interval` (ignored by the stock implant, whose trailer has no third field). WS: `GET /ws` streams `{type, implant_id, data}` events (`implant_connected`/`implant_disconnected`/`hello`/`output`/`error`/`listener_changed`).
