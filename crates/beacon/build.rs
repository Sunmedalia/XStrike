...
//! the beacon exe so its PE metadata looks like an ordinary application
//! (FileDescription, ProductName, CompanyName, FileVersion, …) rather than a
//! stripped, metadata-less binary — same cover identity as the implant. Uses
//! `winres` (shells out to `rc.exe` from the MSVC toolchain). If `rc.exe` can't
//! be located the build does NOT abort — the exe still compiles, just without
//! the resource.
